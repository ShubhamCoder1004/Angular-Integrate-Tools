...
export class LeadscoregeneratorComponent {
  ...
  async ngOnInit() {
    ...
    this.sendLowScoreLeadsForSuggestion()
  }
  suggestions: { Lead_ID: string; Broker_ID: string; suggestion: string }[] =
    [];
  sendLowScoreLeadsForSuggestion() {
    const lowScoreLeads = this.leadData.filter((lead) => lead.score < 70);

    this.leadScoreService.getBrokerSuggestions(lowScoreLeads).subscribe({
      next: (res) => {
